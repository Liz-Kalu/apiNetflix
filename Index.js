...
// La URI viene de las variables de entorno (Vercel -> Settings -> Environment Variables)
...
// Conexión única a MongoDB, cacheada para entornos serverless (Vercel)
...
// Middleware: asegura la conexión antes de procesar cualquier request
app.use(async (req, res, next) => {
  try {
    await conectarDB();
    next();
  } catch (error) {
    res.status(500).json({ mensaje: "No se pudo conectar a la base de datos", error: error.message });
  }
});
...
app.post("/series", async (req, res) => {
  try {
    ...
    res.status(500).json({ mensaje: "Error al guardar la serie", error: error.message });
  }
});
...
app.put("/series/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ mensaje: "Error al actualizar la serie", error: error.message });
  }
});
...
app.get("/peliculas", async (req, res) => {
  try {
    ...
    res.status(500).json({ mensaje: "Error al obtener las peliculas", error: error.message });
  }
});
...
app.post("/peliculas", async (req, res) => {
  try {
    ...
    res.status(500).json({ mensaje: "Error al guardar la pelicula", error: error.message });
  }
});
...
app.put("/peliculas/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ mensaje: "Error al actualizar la pelicula", error: error.message });
  }
});
...
// Solo levantamos el servidor con app.listen en local.
// En Vercel (producción/serverless) NO se debe llamar app.listen: Vercel maneja
// las peticiones invocando el módulo exportado directamente.
